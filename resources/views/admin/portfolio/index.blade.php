@extends('layouts.admin')

@section('title')
    Portfolio | List
@endsection

@section('page-info')
    <div class="br-pagetitle">
        <i class="icon ion-ios-photos-outline"></i>
        <div>
            <h4>Portfolio</h4>
            <p class="mg-b-0">List of all portfolio projects</p>
        </div>
    </div>
@endsection

@section('content')
    <div class="row row-sm">
        <div class="col-sm-12 col-xl-12 mg-t-20 mg-xl-t-0">
            <div class="card">
                <div class="card-body">
                    <div class="bd bd-gray-300 rounded table-responsive">
                        <table class="table my-table table-hover mg-b-0">
                            <thead>
                                <tr>
                                    <th>SL</th>
                                    <th>Title</th>
                                    <th>Sub Title</th>
                                    <th>Project Name</th>
                                    <th>Logo</th>
                                    <th>Description</th>
                                    <th>Caption</th>
                                    <th>Status</th>
                                    <th>Action</th>
                                </tr>
                            </thead>
                            <tbody>
                                @forelse($portfolios as $key => $portfolio)
                                    <tr>
                                        <td>{{ $loop->iteration }}</td>
                                        <td>{{ $portfolio->title }}</td>
                                        <td>{{ $portfolio->sub_title }}</td>
                                        <td>{{ $portfolio->project_name }}</td>
                                        <td>
                                            @if($portfolio->logo_image)
                                                <img src="{{ asset('storage/' . $portfolio->logo_image) }}" 
                                                     alt="{{ $portfolio->title }}" width="50" height="50">
                                            @endif
                                        </td>
                                        <td>{{ Str::limit($portfolio->description, 50) }}</td>
                                        <td>{{ $portfolio->caption }}</td>
                                        <td>
                                            @if($portfolio->status === 'Active')
                                                <span class="badge badge-success">Active</span>
                                            @else
                                                <span class="badge badge-secondary">Inactive</span>
                                            @endif
                                        </td>
                                        <td>
                                            <div class="dropdown">
                                                <a class="btn btn-sm btn-outline-info dropdown-toggle" href="#" 
                                                   role="button" data-toggle="dropdown" aria-expanded="false">
                                                    Action
                                                </a>
                                                <div class="dropdown-menu">
                                                    <a class="dropdown-item" href="{{ route('admin.portfolio.edit', $portfolio->id) }}">
                                                        <i class="fa fa-edit"></i> Edit
                                                    </a>
                                                    <a onclick="deleteRow('{{ route('admin.portfolio.destroy', $portfolio->id) }}')" 
                                                       class="dropdown-item" href="javascript:void(0)">
                                                        <i class="fa fa-trash"></i> Delete
                                                    </a>
                                                </div>
                                            </div>
                                        </td>
                                    </tr>
                                @empty
                                    <tr>
                                        <td colspan="9" class="text-center">No portfolios found</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
                    </div>

                    {{-- Pagination --}}
                    <div class="mt-3">
                        {{ $portfolios->links() }}
                    </div>
                </div>
            </div>
        </div>
    </div>
@endsection
